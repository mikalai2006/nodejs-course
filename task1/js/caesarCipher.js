...
module.exports = class CaesarCipher {
  ...
  createParams() {
    ...
    try {
      ...
      process.stdout.write(
        `Input options : ${JSON.stringify(params)} \r\n`.gray
      );
  ...
  validateParams() {
    ...
    if (this.params.input) {
      if (!fs.existsSync(this.params.input)) {
        this.hError(
          `Error-> No found input file with path = ${this.params.input}`
        );
      }
    }
    if (this.params.output) {
      if (!fs.existsSync(this.params.output)) {
        this.hError(
          `Error-> No found output file with path = ${this.params.output}`
        );
      }
    }
    if (this.params.shift < 0) {
      if (this.params.action == 'encode') {
        this.params.action = 'decode';
        this.params.shift = Math.abs(this.params.shift);
      }
    }
    return this;
  }
  ...
  create() {
    this.createParams().validateParams();
    return this;
  }
};
